...
full.roc <- ROCs(method = "full", T = CA125, Dvec = Dvec.full, ncp = 30,
...
vus("full", T = CA125, Dvec = Dvec.full, ci = TRUE, parallel = TRUE)
```
...
```{r ROCsFI, webgl = TRUE}
fi.roc <- ROCs(method = "fi", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out, 
               ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```
And, for VUS:
```{r vusfi,  eval=FALSE}
vus(method = "fi", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out, ci = TRUE)
```
...
msi.roc <- ROCs(method = "msi", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out, 
                ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```
and compute VUS
```{r vusmsi,  eval=FALSE}
vus(method = "msi", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out, ci = TRUE)
```
...
ipw.roc <- ROCs(method = "ipw", T = CA125, Dvec = Dvec.na, V = V, piEst = pi.out, 
                ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```

```{r vusipw, eval=FALSE}
vus(method = "ipw", T = CA125, Dvec = Dvec.na, V = V, piEst = pi.out, ci = TRUE)
```
...
spe.roc <- ROCs(method = "spe", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out,
                piEst = pi.out, ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```
to provide the plot of ROC surface, and 
```{r vusspe, eval=FALSE}
vus(method = "spe", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.out,
...
knn.roc <- ROCs("knn", CA125, Dvec.na, V = V, A = cbind(CA153, Age), 
                rhoEst = rho.1nn, ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```

```{r vus1nn, eval = FALSE}
vus(method = "knn", T = CA125, Dvec = Dvec.na, V = V, rhoEst = rho.1nn, ci = TRUE,
    parallel = TRUE)
```
...
```{r cvKnn, eval = TRUE}
CVknn(XX, Dvec.na, V, type = "mahala", plot = TRUE)
```
