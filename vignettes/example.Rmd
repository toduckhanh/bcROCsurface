...
rocs(method = "full", diag_test =  EOC$CA125, dise_vec = dise_vec_full, ncp = 30, 
...
## DONE
## 
## CALL: vus_mar(method = "full", diag_test =  CA125, dise_vec = dise_vec.full, ci = TRUE, 
##     parallel = TRUE)
##  
...
rho_out <- rho_mlogit(dise_fact_na ~ CA125 + CA153 + Age, data = EOC, test = TRUE)
```

The following command provides the ROC surface by means of FI esimator:
```{r ROCsFI, webgl = TRUE}
rocs(method = "fi", diag_test =  EOC$CA125, dise_vec = dise_vec_na, veri_stat = EOC$V, 
     rho_est = rho_out, ncp = 30, ellipsoid = TRUE, cpst = c(-0.56, 2.31))
```
And, for VUS:
```{r vusfi,  eval=FALSE}
...
## DONE
## 
## CALL: vus_mar(method = "fi", diag_test = CA125, dise_vec = dise_vec_na, veri_stat = V, rho_est = rho_out, 
##     ci = TRUE)
##  
...
## DONE
## 
## CALL: vus_mar(method = "msi", diag_test =  CA125, dise_vec = dise_vec_na, veri_stat = V, rho_est = rho_out, 
##     ci = TRUE)
##  
...
## DONE
##
## CALL: vus_mar(method = "ipw", diag_test = CA125, dise_vec = dise_vec_na, veri_stat = V, pi_est = pi_out, 
##     ci = TRUE)
## 
...
## DONE
##
## CALL: vus_mar(method = "spe", diag_test = CA125, dise_vec = dise_vec_na, veri_stat = V, rho_est = rho_out, 
##     pi_est = pi_out, ci = TRUE)
##  
...
## DONE
##
## CALL: vus_mar(method = "knn", diag_test = CA125, dise_vec = dise_vec_na, veri_stat = V, rho_est = rho_1nn, 
